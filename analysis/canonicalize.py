...
class Canonicalizer:
    ...
    def run(self, typ: Literal["unit", "dimension"], node: Unit | list, env: Env):
        ...
        normalized = self._resolve_references(self._rewrite_ops(value))

        dimension = simplify(normalized)

        return dimension

    def _rewrite_ops(self, nodes: list[AstNode]) -> list[AstNode | E]:
    ...
    def _resolve_references(self, nodes: list):
        ...
        for node in nodes:
            match node:
                ...
                case E():
                    ...
                    flattened_base = self._resolve_references(base_nodes)
                ...
                case list():
                    res.extend(self._resolve_references(node))
